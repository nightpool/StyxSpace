...
public class SpaceEntityListener extends EntityListener {
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        if (StyxSpace.worldHandler.isSpaceWorld(event.getEntity().getWorld())) {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof Player
                && StyxSpace.worldHandler.isInAnySpace((Player) event.getEntity())) {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            if (SpacePlayerListener.taskid.containsKey(p)) {
                if (StyxSpace.scheduler.isCurrentlyRunning(SpacePlayerListener.taskid.get(p))) {
                    StyxSpace.scheduler.cancelTask(SpacePlayerListener.taskid.get(p));
                }
            }
        }
    }
