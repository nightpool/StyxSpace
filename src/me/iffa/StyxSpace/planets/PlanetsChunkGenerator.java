...
public class PlanetsChunkGenerator extends ChunkGenerator {
	...
	private void loadAllowedBlocks() {
		...
		for (String s : planetConfig.getStringList(
				"styxspace.planets.blocks.cores", null)) {
		...
		for (String s : planetConfig.getStringList(
				"styxspace.planets.blocks.shells", null)) {
			String[] sSplit = s.split("-");
			Material newMat = Material.matchMaterial(sSplit[0]);
			if (newMat.isBlock()) {
				if (sSplit.length == 2) {
	...
	public PlanetsChunkGenerator(Configuration planetConfig, Plugin plugin) {
		...
		this.seed = (long) planetConfig
				.getDouble("styxspace.planets.seed", 0.0);
		this.density = planetConfig.getInt("styxspace.planets.density", 15000);
		minSize = planetConfig.getInt("styxspace.planets.minSize", 4);
		maxSize = planetConfig.getInt("styxspace.planets.maxSize", 20);
		minDistance = planetConfig.getInt("styxspace.planets.minDistance", 10);
		floorBlock = Material.matchMaterial(planetConfig.getString(
				"styxspace.planets.floorBlock", "STATIONARY_WATER"));
		this.floorHeight = planetConfig.getInt("styxspace.planets.floorHeight",
				0);
		minShellSize = planetConfig.getInt("styxspace.planets.minShellSize", 3);
		maxShellSize = planetConfig.getInt("styxspace.planets.maxShellSize", 5);
	...
	public byte[] generate(World world, Random random, int x, int z) {
		...
		for (Planetoid curPl : curSystem) {
			...
			for (int curX = -curPl.radius; curX <= curPl.radius; curX++) {
				...
				if (blkX >= 0 && blkX < 16) {
					...
					int radius = (int) Math.ceil(Math
							.sqrt((curPl.radius * curPl.radius)
						...
						if (blkZ >= 0 && blkZ < 16) {
							...
							for (int curY = -zRadius; curY <= zRadius; curY++) {
								...
								if (xShell || zShell || yShell) {
									retVal[(blkX * 16 + blkZ) * 128 + blkY] = (byte) curPl.shellBlk
											.getId();
								} else {
									retVal[(blkX * 16 + blkZ) * 128 + blkY] = (byte) curPl.coreBlk
											.getId();
								}
							}
						}
		...
		for (int i = 0; i < floorHeight; i++) {
			for (int j = 0; j < 16; j++) {
				for (int k = 0; k < 16; k++) {
					if (i == 0) {
						retVal[j * 2048 + k * 128 + i] = (byte) Material.BEDROCK
								.getId();
					} else {
						retVal[j * 2048 + k * 128 + i] = (byte) floorBlock
								.getId();
					}
				}
			}
	...
	private List<Planetoid> generatePlanets(int x, int z) {
		...
		System.out.println(StyxSpace.prefix + " Made " + planetoids.size()
